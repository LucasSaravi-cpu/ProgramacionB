...
public class Uso_automovil {
 ...
 // Clases de automóviles que queremos filtrar
    ...
    Map<String, Automovil> mapaAutos = new HashMap<>();
    for (Automovil auto : listaAutomoviles) {
        mapaAutos.put(auto.getPatente(), auto);
    }
    
 // Actualizar el kilometraje de un automóvil (ejemplo)
    String patenteActualizar = "ABC123";
    int nuevoKilometraje = 50000;
    if (mapaAutos.containsKey(patenteActualizar)) {
        Automovil autoActualizar = mapaAutomoviles.get(patenteActualizar);
        autoActualizar.setKilometraje(nuevoKilometraje);
    } else {
        // Si no existe la patente, agregar un automóvil sin dueño y con motor desconocido
        Automovil nuevoAuto = new Automovil(patenteActualizar, "Sin dueño", new Motor(0, "Desconocido"));
        nuevoAuto.setKilometraje(nuevoKilometraje);
        mapaAutos.put(patenteActualizar, nuevoAuto);
    }

    // Imprimir los automóviles en el mapa
    for (Automovil auto : mapaAutomoviles.values()) {
