...
public class VentanaTrabajos extends JFrame {
	...
	private void crearTrabajo() {
		...
		Date fechaExposicion = (Date) spinnerFechaExposicion.getValue();
    	Calendar calendar = Calendar.getInstance();
    	calendar.setTime(fechaExposicion);
    	int diaDelAnio = calendar.get(Calendar.DAY_OF_YEAR);
	    
	    if (rdbtnInvestigacion.isSelected()) {
	    	AreaTematica area = new AreaTematica(cbAreaTematica.getSelectedIndex(),"");
	    	
	        
	    	TrabajoInvestigacion trabajoInv= new TrabajoInvestigacion(textTitulo.getSelectedText(),diaDelAnio,area,checkPapers.isSelected(),0);
	        ...
	        if (rdbtnEmpresarial.isSelected()){
	        	...
	        	AreaTematica area = new AreaTematica(cbAreaTematica.getSelectedIndex(),"");
	        	TrabajoEmpresarial trabajoEmp = new TrabajoEmpresarial(textTitulo.getSelectedText(),diaDelAnio,area,checkComercial.isSelected(),(int)spinnerAnioCreacion.getValue(),0);
	    	...
	    	if (rdbtnEstudiantil.isSelected()) {
	    		...
	    		AreaTematica area = new AreaTematica(1,"");
	        	TrabajoEstudiantil trabajoEst = new TrabajoEstudiantil(textTitulo.getSelectedText(),diaDelAnio,area,checkFinalCarrera.isSelected(),(int) spinnerAnioCarrera.getValue(),0,(Alumno)cbAlumno.getSelectedItem());
	        	double tarifa = trabajoEst.CalcularTarifa();
		    	trabajoEst.setVALOR(tarifa);
