...
public class ClinicController {
	...
	public String costsReports() {
	  ...
	  //Hash sugerido para obtener el total por medico
	    HashMap<String, Double> professionalCosts = new HashMap<>();
	    ...
	    for (Patient patient : patients.values()) {
	        ...
	        for (Prescription prescription : prescriptions) {
	            if (prescription.getPatient() == patient) {
	                ...
	                total += costo;
	                sb.append("\t" + prescription.toString() + "\t" + /*prescription.tipoprescription() +*/ "\t" +"Costo Total "+ costo + "\n");
	                
	                // Acumula el costo por profesional
	                String professionalName = prescription.getProfessional();
	                professionalCosts.put(professionalName, professionalCosts.getOrDefault(professionalName, 0.0) + costo);
	            }
	        }
	    }
	    ...
	    // Mostrar los Totales por profesional
	    for (Map.Entry<String, Double> entry : professionalCosts.entrySet()) {
	        String professionalName = entry.getKey();
	        Double total = entry.getValue();
	        sb.append(professionalName + ": " + total + "\n");
	    }
